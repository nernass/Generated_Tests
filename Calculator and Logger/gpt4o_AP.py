...
class TestIntegration:
    ...
    def test_logger_exception_handling(self, setup):
        calculator, logger, log_file = setup
        with patch.object(logger, 'log', side_effect=Exception("Logging failed")):
            with pytest.raises(Exception, match="Logging failed"):
                calculator.add(10, 5)
    
    def test_edge_case_zero_addition(self, setup):
        calculator, _, log_file = setup
