...
class TestIntegrationGameWorldAndPlayer:
    ...
    def test_partial_failure_game_world_get_location_info_fails(self, player, game_world):
        game_world.get_location_info = MagicMock(return_value=None)
        result = player.pick_up("mushroom")
        assert result == False
        assert player.inventory == []
        assert game_world.locations["forest"]["items"] == ["mushroom", "stick"]

    def test_partial_failure_game_world_remove_item_fails(self, player, game_world):
        game_world.remove_item = MagicMock(return_value=False)
        result = player.pick_up("mushroom")
        assert result == False
        assert player.inventory == []
        assert game_world.locations["forest"]["items"] == ["mushroom", "stick"]
